...
const string strMessageMagic = "RuBiX Signed Message:\n";
...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction &tx, bool fLimitFree,
                        bool* pfMissingInputs, bool fRejectrubixFee, bool ignoreFees, bool fFixSpentCoins)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
...

    {
        ...
        if (fRejectrubixFee && nFees > MIN_RELAY_TX_FEE * 10000)
            return error("AcceptableInputs: : rubix fees %s, %d > %d",
                         hash.ToString(),
...
bool AcceptableInputs(CTxMemPool& pool, const CTransaction &txo, bool fLimitFree,
                         bool* pfMissingInputs, bool fRejectrubixFee, bool isDSTX)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
        *pfMissingInputs = false;
...

    {
        ...
        if (fRejectrubixFee && nFees > txMinFee * 10000)
            return error("AcceptableInputs: : rubix fees %s, %d > %d",
                         hash.ToString(),
                         nFees, MIN_RELAY_TX_FEE * 10000);
...
bool CMerkleTx::AcceptToMemoryPool(bool fLimitFree, bool fRejectrubixFee, bool ignoreFees)
{
    return ::AcceptToMemoryPool(mempool, *this, fLimitFree, NULL, fRejectrubixFee, ignoreFees);
}
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = HashBmw512(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        hashKey = HashBmw512(BEGIN(hashKey), END(hashKey));
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        uint256 hash = HashBmw512(vRecv.begin(), vRecv.begin() + nMessageSize);
