...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "RuBiX.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "RuBiX.conf";
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "RuBiXd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong RuBiX will not work properly.");
