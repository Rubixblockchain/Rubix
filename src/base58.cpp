...
bool DecodeBase58Check(const char* psz, std::vector<unsigned char>& vchRet) {
    ...
    uint256 hash = HashBmw512(vchRet.begin(), vchRet.end()-4);
...
namespace {
    class CRuBiXAddressVisitor : public boost::static_visitor<bool> {
    private:
        CRuBiXAddress *addr;
    public:
        CRuBiXAddressVisitor(CRuBiXAddress *addrIn) : addr(addrIn) { }
...
bool CRuBiXAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CRuBiXAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CRuBiXAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CRuBiXAddressVisitor(this), dest);
}

bool CRuBiXAddress::IsValid() const {
...
CTxDestination CRuBiXAddress::Get() const {
    if (!IsValid())
...
bool CRuBiXAddress::GetKeyID(CKeyID &keyID) const {
...
bool CRuBiXAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CRuBiXSecret::SetKey(const CKey& vchSecret) {
...
CKey CRuBiXSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CRuBiXSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CRuBiXSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CRuBiXSecret::SetString(const std::string& strSecret) {
