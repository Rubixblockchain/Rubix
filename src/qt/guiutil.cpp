...
{
    widget->setMaxLength(RuBiXAddressValidator::MaxAddressLength);
    widget->setValidator(new RuBiXAddressValidator(parent));
...
bool parseRuBiXURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("RuBiX"))
        return false;
...
bool parseRuBiXURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rubix:// to RuBiX:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("rubix://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "RuBiX:");
    }
    QUrl uriInstance(uri);
    return parseRuBiXURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "RuBiX.lnk";
}
...
{
    return GetAutostartDir() / "RuBiX.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a RuBiX.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=RuBiX\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the RuBiX app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add RuBiX app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("RuBiX-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  RuBiX-qt [" + tr("command-line options") + "]                     " + "\n";
